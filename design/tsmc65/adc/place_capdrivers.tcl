# Cap Driver Power Domain Placement Script for FRIDA ADC TSMC65
...
# ADC core area dimensions (from config.mk: 60x60 with 2μm margins)
set core_area_x1 2
set core_area_y1 2
set core_area_x2 58
set core_area_y2 58
...
foreach capdriver_inst $capdriver_instances {
    ...
    set xor_width 1.44   ; # TSMC65 minimal XOR cell width
    set xor_height 2.88  ; # TSMC65 standard cell height
    set spacing_x 2.16   ; # 1.5x cell width for routing space
    set spacing_y 3.6    ; # 1.25x cell height for routing space
    ...
    for {set i 0} {$i < 16} {incr i} {
        ...
        # Place XOR gate with TSMC65 cell type
        if {[catch {place_inst -name $xor_inst_name -origin [list $xor_x $xor_y] -orientation R0 -status PLACED -cell CLKXOR2X1LVT} result]} {
...
# TSMC65 standard cell height = 2.88μm, so track pitch ≈ 1.8μm
...
set track_3rd_from_bottom [expr $core_bottom + (3 * $track_pitch)]  # ~7.4μm
set track_3rd_from_top [expr $core_top - (3 * $track_pitch)]        # ~52.6μm

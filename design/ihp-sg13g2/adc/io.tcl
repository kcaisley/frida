...
# Bottom - All control signals (will use IO_PLACER_H = Metal2)
set_io_pin_constraint -pin_names {seq_init seq_samp seq_comp seq_update} -region bottom:10-60  -group -order
set_io_pin_constraint -pin_names {en_init en_samp_p en_samp_n en_comp en_update} -region bottom:10-60 -group -order
set_io_pin_constraint -pin_names {dac_diffcaps comp_out} -region bottom:10-60 -group -order
set_io_pin_constraint -pin_names {dac_mode} -region bottom:10-60 -group -order

# Top - Analog pins centered (150um die width, center at 75um)
set_io_pin_constraint -pin_names {vin_p vin_n} -region top:70-80 -group -order
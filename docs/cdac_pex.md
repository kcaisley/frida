...
The historical held-level histogram used a clock-derived averaging window of approximately
...
This archived comparison is not regenerated by the sequence runners; the
reusable sampling-noise analyzer and plotter remain available.
...
The `frida1_sequence` and `frida2_sequence` family runners
...
`flow.caparray.subckt.CapArray` now generates both the ideal and LVS capacitor
...
Only the leaf view changes: `flow.caparray.unit.MomCap` contains an ideal
...
`CapArrayConfig.unit_cap` retains its meaning as the complete stack's logical
...
The calculation is automated by `flow.caparray.pex`. Historical ADC extraction
must call `parse_adc_caparray_pex(..., stage_count=16,

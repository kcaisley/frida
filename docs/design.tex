...
%% ============================================================
\begin{frame}
  \frametitle{OpenROAD Analog Constraint Matrix}
  \tiny
  \begin{center}
    \begin{tabular}{p{2.6cm}|p{1.2cm}|p{3.9cm}|p{4.0cm}}
      \hline
      \textbf{Constraint Idea} & \textbf{Status} & \textbf{OpenROAD Mechanism} & \textbf{Notes} \\
      \hline
      IO side / region / grouping &
      Supported &
      \texttt{set\_io\_pin\_constraint}, \texttt{exclude\_io\_pin\_region}, \texttt{place\_pin}, \texttt{place\_pins} &
      Includes mirrored IO-pair support for boundary placement \\
      \hline
      Block boundary / utilization / aspect ratio &
      Supported &
      \texttt{initialize\_floorplan} &
      ...
      Partial &
      \texttt{optimize\_mirroring} (weak) &
      No first-class pairwise analog symmetry intent \\
      \hline
      Net width/spacing overrides &
      Supported &
      \texttt{create\_ndr}, \texttt{assign\_ndr} &
      Good fit for high-current or low-resistance nets \\
      \hline
      Shielded routing &
      Missing &
      ODB shield wire types exist &
      No high-level GRT/DRT command for automatic shield routing \\
      \hline
      Route prioritization &
      Partial &
      \texttt{global\_route -critical\_nets\_percentage}, \texttt{set\_nets\_to\_route} &
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item LEF abstract generation and checking soon? (\href{https://github.com/The-OpenROAD-Project/OpenROAD/issues/4872}{OpenROAD issue \#4872}).

...
\begin{frame}
  ...
  \centering
  \begin{tikzpicture}
    \node[inner sep=0pt] (adc) {\includegraphics[height=0.72\textheight,keepaspectratio]{../images/frida_adc.png}};
...
\begin{frame}
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.13\linewidth}
    ...
    SAMP & \pin{seq_samp} & Hold high for 20 ns to acquire both differential inputs. \\
    COMP & \pin{seq_comp} & Apply sixteen 2.5 ns high windows on a 5 ns period. \\
    LOGIC & \pin{seq_update} & Pulse once with INIT, then apply fifteen delayed 2.5 ns update windows. \\
    \bottomrule
  \end{tabular}
\end{frame}

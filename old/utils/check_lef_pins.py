...
class LEFParser:
    ...
    def parse_tech_lef(self, tech_lef_path):
        ...
        for layer_match in re.finditer(
            r"LAYER\s+(\w+)\s+(.*?)END\s+\1", content, re.DOTALL
        ):
            ...
            if "Metal" in layer_name or "M" in layer_name:
                ...
                pitch_match = re.search(
                    r"PITCH\s+([\d.]+)(?:\s+([\d.]+))?", layer_content
                )
                if pitch_match:
                    x_pitch = float(pitch_match.group(1))
                    y_pitch = (
                        float(pitch_match.group(2)) if pitch_match.group(2) else x_pitch
                    )
                ...
                if spacing_match:
                    self.tech_info[layer_name]["spacing"] = float(
    ...
    def parse_macro_lef(self, macro_lef_path):
        ...
        for macro_match in re.finditer(
            r"MACRO\s+(\w+)\s+(.*?)END\s+\1", content, re.DOTALL
        ):
            macro_name = macro_match.group(1)
            macro_content = macro_match.group(2)

            self.macros[macro_name] = {"pins": {}, "size": None}

            # Extract macro size
            size_match = re.search(r"SIZE\s+([\d.]+)\s+BY\s+([\d.]+)", macro_content)
            if size_match:
                self.macros[macro_name]["size"] = (
            ...
            for pin_match in re.finditer(
                r"PIN\s+(\w+)\s+(.*?)END\s+\1", macro_content, re.DOTALL
            ):
                ...
                for port_match in re.finditer(
                    r"PORT\s+(.*?)END", pin_content, re.DOTALL
                ):
                    ...
                    for layer_match in re.finditer(r"LAYER\s+(\w+)\s+;", port_content):
                        ...
                        for rect_match in re.finditer(rect_pattern, port_content):
                            ...
                            pin_info["layers"].append(
                                {"layer": layer_name, "rect": (x1, y1, x2, y2)}
                            )
        ...
        for macro_name, macro_info in self.macros.items():
            print(
                f"  {macro_name}: {len(macro_info['pins'])} pins, size {macro_info['size']}"
            )
            ...
            for pin_name, pin_info in macro_info["pins"].items():
                ...
                for layer_info in pin_info["layers"]:
                    ...
                    if layer not in self.tech_info:
                        issues.append(
                            f"{macro_name}.{pin_name}: Layer {layer} not found in technology file"
                        )
                        continue
                    ...
                    if "pitch" not in tech_layer:
                        issues.append(
                            f"{macro_name}.{pin_name}: No pitch info for layer {layer}"
                        )
                        continue
                    ...
                    if (
                        x_grid_error > tolerance
                        and (x_pitch - x_grid_error) > tolerance
                    ):
                        issues.append(
                            f"{macro_name}.{pin_name}.{layer}: X center {center_x} not on {x_pitch} grid (error: {x_grid_error})"
                        )

                    if (
                        y_grid_error > tolerance
                        and (y_pitch - y_grid_error) > tolerance
                    ):
                        issues.append(
                            f"{macro_name}.{pin_name}.{layer}: Y center {center_y} not on {y_pitch} grid (error: {y_grid_error})"
                        )
                    ...
                    if "width" in tech_layer:
                        ...
                        if width < min_width:
                            issues.append(
                                f"{macro_name}.{pin_name}.{layer}: Width {width} < minimum {min_width}"
                            )
                        if height < min_width:
                            issues.append(
                                f"{macro_name}.{pin_name}.{layer}: Height {height} < minimum {min_width}"
                            )
                    ...
                    if area < 0.01:  # Minimum 0.01 square microns
                        issues.append(
                            f"{macro_name}.{pin_name}.{layer}: Area {area} too small"
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Check LEF pin legalization for IHP-SG13G2"
    )

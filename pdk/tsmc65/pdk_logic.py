"""HDL21 compile logic for TSMC65."""
...
class Tsmc65Walker(h.HierarchyWalker):
    ...
    def visit_primitive_call(self, call: h.PrimitiveCall) -> h.Instantiable:
        ...
        if call.prim in {
            ...
            # TODO: add passive/ diode/ bjt wrappers for TSMC65.
            return call
        return call
    ...
    def mos_module(self, params: MosParams) -> h.ExternalModule:
        ...
        if mod is None:
            raise RuntimeError(
                f"No TSMC65 MOS module for type={mostype}, vth={mosvth}, family={mosfam}"
            )
        return mod

    def mos_params(self, params: MosParams, modname: str):
        from .pdk_data import Tsmc65MosParams
...
def compile(src: h.Elaboratables) -> None:
    """Compile source into TSMC65 modules."""

...
class Frida:
    ...
    generated by the FPGA sequencer.
    ...
    def __init__(self, daq: Dut):
        ...
        Args:
            daq: Initialized Basil Dut object (from daq.yaml)
        """
        self.daq = daq
        ...
        reg_path = Path(__file__).parent / "registers.yaml"
    ...
    def init(self) -> None:
        ...
        Performs:
            ...
            3. Release reset
            4. Load default SPI configuration
        """
    ...
    def set_register(self, name: str, value: int) -> None:
        ...
        Args:
            name: Register name from registers.yaml
            value: Value to set
        """
    ...
    def _configure_sequencer(
        ...
        Sets up the 4 clock tracks (CLK_INIT, CLK_SAMP, CLK_COMP, CLK_LOGIC)
        for one conversion cycle.
        ...
        """
        from .sequencer import generate_conversion_sequence
    ...
    def run_conversions(self, n_conversions: int, repetitions: int = 1) -> np.ndarray:
        ...
        # Read data from FIFO
        n_bytes = n_conversions * repetitions * 2  # 16 bits per conversion
        raw_data = self.daq["fifo"].get_data(n_bytes)

        # Reshape into conversion results
        data = np.frombuffer(raw_data, dtype=np.uint16)
        data = data.reshape(repetitions, n_conversions)

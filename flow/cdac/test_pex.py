...
def test_write_comparison_table_has_34_rows_for_16_bits(tmp_path: Path) -> None:
    ...
    for stage in range(16):
        lines.extend(
            (
                f"c{stage}m ( CAP_TOPPLATE CAP_BOTPLATE_MAIN<{stage}> ) capacitor c={stage + 2}f",
                f"c{stage}d ( CAP_TOPPLATE CAP_BOTPLATE_DIFF<{stage}> ) capacitor c=1f",
            )
        )
    ...
    assert list(json.loads(json_path.read_text(encoding="utf-8"))) == [
        *(f"main[{stage}]_ff" for stage in range(16)),
        *(f"diff[{stage}]_ff" for stage in range(16)),
        "main_plus_diff_total_ff",
        "shunt_dc_ff",
    ]


def test_legacy_pin_order_is_translated_once_to_c0_first(tmp_path: Path) -> None:
    source = tmp_path / "legacy.pex.netlist"
    source.write_text(
        """c0m ( CAP_TOPPLATE CAP_BOTPLATE_MAIN<0> ) capacitor c=2f
c0d ( CAP_TOPPLATE CAP_BOTPLATE_DIFF<0> ) capacitor c=1f
c1m ( CAP_TOPPLATE CAP_BOTPLATE_MAIN<1> ) capacitor c=8f
c1d ( CAP_TOPPLATE CAP_BOTPLATE_DIFF<1> ) capacitor c=3f
""",
        encoding="utf-8",
    )

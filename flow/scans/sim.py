...
class SimAWG:
    ...
    async def start_sin(
        ...
        raise NotImplementedError(
            "Sinusoidal drive not yet implemented for simulation. "
            "Use set_differential() with a manual sweep instead."
        )
...
class SimPSU:
    ...
    async def set_voltage(self, v: float) -> None:
        if abs(v - self._vdd) > 0.001:
            logger.warning(
                "SimPSU: VDD is fixed at %.3fV in the SPICE netlist; "
                "ignoring request to change to %.3fV",
                self._vdd, v,
            )
...
def _adc_port_order() -> list[str]:
    ...
    ports = [
        "seq_init", "seq_samp", "seq_comp", "seq_update",
        "comp_out",
        "en_init", "en_samp_p", "en_samp_n", "en_comp", "en_update",
        "dac_mode", "dac_diffcaps",
    ]

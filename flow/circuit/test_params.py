...
from .params import supply_voltage, temperature_c
...
def test_supply_voltage_uses_pdk_metadata(tech_name: str, expected_typ: float) -> None:
    """Resolve VDD from per-PDK supply metadata."""
    ...
    voltage = supply_voltage(Corner.TYP, tech_name=tech_name)
    assert float(voltage) == pytest.approx(expected_typ)

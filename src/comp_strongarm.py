def subcircuit():
    ...
    topology = {
        'subckt': 'comp_strongarm',
        'ports': {'in_p': 'I', 'in_n': 'I', 'out_p': 'O', 'out_n': 'O', 'clk': 'I', 'clk_b': 'I', 'vdd': 'B', 'vss': 'B'},
        'devices': {
            # Input differential pair
            'MNinn': {'dev': 'nmos', 'pins': {'d': 'midn', 'g': 'in_n', 's': 'tail', 'b': 'vss'}},
            'MNinp': {'dev': 'nmos', 'pins': {'d': 'midp', 'g': 'in_p', 's': 'tail', 'b': 'vss'}},

            # Tail transistor (clock-controlled)
            'MNtail': {'dev': 'nmos', 'pins': {'d': 'tail', 'g': 'clk', 's': 'vss', 'b': 'vss'}},
...
def testbench():
    ...
    topology = {
        'testbench': 'tb_comp_strongarm',
        'devices': {
            'Vvdd': {'dev': 'vsource', 'pins': {'p': 'vdd', 'n': 'gnd'}, 'wave': 'dc', 'dc': 1.0},
            'Vvss': {'dev': 'vsource', 'pins': {'p': 'vss', 'n': 'gnd'}, 'wave': 'dc', 'dc': 0.0},
            'Xdut': {'dev': 'comp_strongarm', 'pins': {'in_p': 'in_p', 'in_n': 'in_n', 'out_p': 'out_p', 'out_n': 'out_n', 'clk': 'clk', 'clk_b': 'clk_b', 'vdd': 'vdd', 'vss': 'vss'}}
        },
        'analyses': {
            'mc1': {
                'type': 'montecarlo',

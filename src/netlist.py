...
techmap = {
    'tsmc65': {
        ...
        'corners': {'tt': 'tt_lib', 'ss': 'ss_lib', 'ff': 'ff_lib', 'sf': 'sf_lib', 'fs': 'fs_lib', 'mc': 'mc_lib'}
    },
    'tsmc28': {
        'libs': [
            {
        ...
        'corners': {'tt': 'att_pt', 'ss': 'ass_ps', 'ff': 'aff_pf', 'sf': 'asf_ps', 'fs': 'afs_pf', 'mc': 'local_mc'},
        'noise_models': {
            'worst': {
    ...
    'tower180': {
        ...
        'corners': {'tt': 'NOM', 'ss': 'SLOW', 'ff': 'FAST', 'sf': 'SLOWFAST', 'fs': 'FASTSLOW', 'stat': 'STAT'}
    }
}
...
scalemap = {
    ...
    'pulse': {'v1': 'voltage', 'v2': 'voltage', 'td': 'time', 'tr': 'time', 'tf': 'time', 'pw': 'time', 'per': 'time'},
}
...
def compact_json(obj, indent=2, lvl=0):
    ...
    is_leaf = lambda v: not any(isinstance(x, (dict, list)) for x in (v.values() if isinstance(v, dict) else v if isinstance(v, list) else []))

    if isinstance(obj, dict) and obj:
        if is_leaf(obj):
            return '{' + ', '.join(f'"{k}": {json.dumps(v)}' for k, v in obj.items()) + '}'
        return '{\n' + ',\n'.join(f'{pad1}"{k}": {compact_json(v, indent, lvl+1)}' for k, v in obj.items()) + f'\n{pad}}}'
    if isinstance(obj, list) and obj:
        if is_leaf(obj):
            return '[' + ', '.join(json.dumps(v) for v in obj) + ']'
        return '[\n' + ',\n'.join(f'{pad1}{compact_json(v, indent, lvl+1)}' for v in obj) + f'\n{pad}]'
...
def expand_sweeps(topology: Dict, sweep: Dict) -> List[Dict]:
    """
    ...
    for sweep_spec in sweep['sweeps']:
        ...
        param_lists = {p: sweep_spec[p] for p in ['w', 'l', 'nf', 'type'] if p in sweep_spec}
...
def map_to_technology(topology: Dict, tech: str, techmap: Dict) -> Dict:
    """
...
def scale_testbench_values(topology: Dict, tech: str, techmap: Dict) -> Dict:
...
def add_automatic_fields(topology: Dict, tech: str, techmap: Dict, subckt_topology: Dict = None, corner: str = 'tt') -> Dict:
    """
        ...
        if dut_instance:
            ...
            for dev_name, dev_info in subckt_topology.get('devices', {}).items():
                if 'meta' in dev_info:
                    ...
                    oppoint = ' '.join(f"{hier_name}:{p}" for p in
                                       ['vth', 'gds', 'gm', 'vdsat', 'cdb', 'cdg', 'csg', 'csb'])
...
def topology_to_spice(topology: Dict, mode: str = 'subcircuit') -> str:
    """
    ...
    if mode == 'subcircuit':
        ...
        for dev_name, dev_info in devices.items():
            ...
            if dev == 'vsource':
                ...
                if wave == 'dc':
                    ...
                    pwl_str = ' '.join([format_value(v, 's' if i%2==0 else 'V') for i, v in enumerate(points)])
                    line += f" PWL({pwl_str})"
        ...
        if analyses:
            ...
            if mc_analysis and tran_analysis:
                ...
                step = format_value(tran_spec.get('step', 1e-9), 's') if 'step' in tran_spec else None

                lines.append(f"{mc_name} montecarlo numruns={mc_spec.get('numruns', 20)} " +
                ...
                if 'strobeperiod' in tran_spec:
                    tran_line += f" strobeperiod={format_value(tran_spec['strobeperiod'], 's')}"
                if 'noisefmax' in tran_spec:
                    tran_line += f" noisefmax={format_value(tran_spec['noisefmax'], 'Hz')}"
...
def generate_netlist_filename(topology: Dict, tech: str, sweep_spec: Dict = None) -> str:
...
def generate_subcircuits(circuit_module, output_dir: Path, tech_list: List[str] = None):
    """Generate subcircuit netlists."""
    topology, sweep = circuit_module.subcircuit()

    if tech_list is None:
        tech_list = sweep.get('tech', ['tsmc65'])

    # Expand sweeps
    topologies = expand_sweeps(topology, sweep)

    print(f"Generated {len(topologies)} parameter combinations")
    ...
    for tech in tech_list:
        ...
        print(f"  Generated {len(topologies)} netlists")

    print(f"\nTotal: {count} netlists generated")


def generate_testbenches(circuit_module, subckt_module, output_dir: Path, tech_list: List[str] = None, corner: str = 'tt'):
    ...
    for tech in tech_list:
        ...
        complete_tb = add_automatic_fields(scaled_tb, tech, techmap, subckt_topology, corner)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Generate SPICE netlists from Python specifications')
    parser.add_argument('mode', choices=['subckt', 'tb'], help='Generation mode: subckt or tb')
    parser.add_argument('circuit', type=Path, help='Circuit Python file (e.g., src/samp_tgate.py)')
    parser.add_argument('-o', '--output', type=Path, required=True, help='Output directory')
    parser.add_argument('-t', '--tech', nargs='+', help='Technology list (default: from circuit file)')
    parser.add_argument('-c', '--corner', default='tt', help='Process corner for testbenches (default: tt)')
    ...
    if args.mode == 'subckt':
        ...
        generate_testbenches(circuit_module, circuit_module, args.output, args.tech, args.corner)

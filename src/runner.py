params = {
    ...
    "CDAC": {
        ...
        "unit_capacitance": 0.5e-15, # roughly 1um^2 MOM cap
...
array_sizes = [8,12,16,20] # number of capacitor bit positions, this is used to find percentage settling. More than 20 doesn't make sense as
settling_times = [0, 5e-9, 10e-9] # The CDAC time constant. Based on 2X idealS simulation, I expect the settling time to be on order of single digit ns, so ~ 5ns. We assume drivers are sized to ensure equal settling time on each node.
# Wait, even if the settling time constant is the same for all caps, for large voltage steps (common in MSBs) the error in absolute mV will be large

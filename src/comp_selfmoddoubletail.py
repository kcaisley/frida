def subcircuit():
    ...
    topology = {
        ...
        'devices': {
            ...
            'MNtailCN': {'dev': 'nmos', 'pins': {'d': 'tailCN', 'g': 'midn', 's': 'vss', 'b': 'vss'}},
            'MNtailCP': {'dev': 'nmos', 'pins': {'d': 'tailCP', 'g': 'midp', 's': 'vss', 'b': 'vss'}},
...
def testbench():
    ...
    topology = {
        'testbench': 'tb_comp_selfmoddoubletail',
        'devices': {
            'Vvdd': {'dev': 'vsource', 'pins': {'p': 'vdd', 'n': 'gnd'}, 'wave': 'dc', 'dc': 1.0},
            'Vvss': {'dev': 'vsource', 'pins': {'p': 'vss', 'n': 'gnd'}, 'wave': 'dc', 'dc': 0.0},
            'Xdut': {'dev': 'comp_selfmoddoubletail', 'pins': {'in_p': 'in_p', 'in_n': 'in_n', 'out_p': 'out_p', 'out_n': 'out_n', 'clk': 'clk', 'clk_b': 'clk_b', 'vdd': 'vdd', 'vss': 'vss'}}
        },
        'analyses': {
            'mc1': {
                'type': 'montecarlo',

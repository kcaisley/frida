...
def convert_gds_to_lef(
    gds_path, out_file, lyt_file_path, blockage_layers=None, cell_name=None
):
    """
    ...
    with open(out_file, "w") as file:
        ...
        for pin_name, pin_x, pin_y in pin_texts:
            ...
            for i, (x1, y1, x2, y2) in enumerate(m4_rectangles):
                ...
                dist = (
                    (pin_x - rect_center_x) ** 2 + (pin_y - rect_center_y) ** 2
                ) ** 0.5
            ...
            if best_rect:
                ...
                print(
                    f"        RECT {x1 / 1000:.3f} {y1 / 1000:.3f} {x2 / 1000:.3f} {y2 / 1000:.3f} ;",
                    file=file,
                )
                print("    END", file=file)
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print(
            "Usage: python gds2lef.py <input.gds> <output.lef> <layer_map.lyt> [blockage_layers]"
        )
        print(
            "  blockage_layers: comma-separated list of layer names to export as single blockage rectangles"
        )
    ...
    try:
        convert_gds_to_lef(
            gds_path, out_file, lyt_file_path, blockage_layers, cell_name
        )
